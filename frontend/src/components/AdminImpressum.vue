<template>
  <BaseCard>
    <div class="flex items-center justify-between mb-4">
      <h5 class="text-lg font-semibold">Impressum</h5>
    </div>
    <p class="text-sm text-gray-500 mb-4">Das Impressum wird im Footer verlinkt.</p>
      ...
      <template v-else>
        <div class="field">
          <FloatLabel variant="on">
            <InputText id="impressum-url" v-model="url" class="w-full" placeholder="https://…" />
            <label for="impressum-url">URL des Impressums</label>
          </FloatLabel>
        </div>
      </template>
...
const submit = async () => {
  ...
  try {
    await apiClient.patch('/admin/einstellung', {
      impressumModus: modus.value,
      impressumInhalt: inhalt.value,
      impressumUrl: url.value
    })
    ...
    toast.add({
      severity: 'error',
      summary: 'Fehler',
      detail: 'Impressum konnte nicht gespeichert werden.',
      life: 3000
    })
  } finally {
...
const confirmReset = () => {
  confirm.require({
    message:
      'Das Impressum wird gelöscht, ein hinterlegter externer Link entfernt und der Modus auf "HTML-Inhalt" zurückgesetzt.',
